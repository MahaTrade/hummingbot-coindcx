...
class RateOracleTest(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        ...
        RateOracle.coingecko_extra_token_ids = []
        super().tearDownClass()
    ...
    def test_get_rate_coingecko(self, mock_api):
        coingecko_extra_token_ids = ["safemoney", "vfox"]
        RateOracle.coingecko_extra_token_ids = coingecko_extra_token_ids
        ...
        url = RateOracle.coingecko_extra_tokens_url.format(",".join(coingecko_extra_token_ids), "USD")
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        mock_api.get(regex_url, body=json.dumps(Fixture.CoinGeckoExtraTokens), repeat=True)
        ...
        self._assert_rate_dict(rates)
        self.assertEqual(Decimal(str(Fixture.CoinGeckoExtraTokens[0]["current_price"])), rates["VFOX-USD"])

    @aioresponses()
    def test_coingecko_extra_token_rates_only_requested_when_extra_tokens_configured(self, mock_api):
        coingecko_extra_token_ids = ["safemoney", "vfox"]
        ...
        rates = self.async_run_with_timeout(RateOracle.get_coingecko_extra_token_prices("uds"))
        self.assertEqual(0, len(rates))

        RateOracle.coingecko_extra_token_ids = coingecko_extra_token_ids

        rates = self.async_run_with_timeout(RateOracle.get_coingecko_extra_token_prices("usd"))
        self.assertEqual(Decimal(str(Fixture.CoinGeckoExtraTokens[0]["current_price"])),
                         rates[f"{Fixture.CoinGeckoExtraTokens[0]['symbol'].upper()}-USD"])
        self.assertEqual(Decimal(str(Fixture.CoinGeckoExtraTokens[1]["current_price"])),
                         rates[f"{Fixture.CoinGeckoExtraTokens[1]['symbol'].upper()}-USD"])
