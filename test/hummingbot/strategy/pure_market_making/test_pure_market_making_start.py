...
class PureMarketMakingStartTest(unittest.TestCase):
    ...
    def test_strategy_creation_with_multi_market_price_source(self):
        c_map.get("price_source").value = "multi_market"
        c_map.get("price_source_exchanges_and_markets").value = ["binance:TOK1-TOK2", "binance:TOK2-TOK3"]

        strategy_start.start(self)
        self.assertEqual("TOK1-TOK3", self.strategy.asset_price_delegate.trading_pair)
        self.assertEqual("binance_PaperTrade", self.strategy.asset_price_delegate.all_markets[0].display_name)
