...
from hummingbot.connector.exchange.alpha_point import alpha_point_constants as AP_CONSTANTS
...
class AlphaPointExchangeTests:
    class ExchangeTests(AbstractExchangeConnectorTests.ExchangeConnectorTests, ABC):
        ...
        def all_symbols_url(self):
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_PRODUCTS_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?") + ".*")
            return regex_url

        @property
        def latest_prices_url(self) -> Pattern[str]:
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_GET_L1_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
            return regex_url

        @property
        def network_status_url(self) -> Pattern[str]:
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_PING_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
            return regex_url
        ...
        def order_creation_url(self) -> str:
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_ORDER_CREATION_ENDPOINT)
            return url

        @property
        def balance_url(self) -> str:
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_ACC_POSITIONS_ENDPOINT)
            return url
        ...
        def balance_event_websocket_update(self) -> Dict[str, Union[str, int, float]]:
            return {
                "i": 10,
                "m": 3,
                "n": AP_CONSTANTS.WS_ACC_POS_EVENT,
        ...
        def validate_auth_credentials_present(self, request_call: RequestCall):
            ...
            self.assertEqual(request_headers[AP_CONSTANTS.API_KEY_FIELD], self.api_key)
            self.assertEqual(request_headers[AP_CONSTANTS.SIGNATURE_FIELD], self.signature)
            self.assertEqual(request_headers[AP_CONSTANTS.USER_ID_FIELD], str(self.user_id))
            self.assertEqual(request_headers[AP_CONSTANTS.NONCE_FIELD], self.nonce)

        def validate_order_creation_request(self, order: InFlightOrder, request_call: RequestCall):
            request_data = json.loads(request_call.kwargs["data"])
            self.assertEqual(request_data[AP_CONSTANTS.INSTRUMENT_ID_FIELD], self.pair_id)
            self.assertEqual(request_data[AP_CONSTANTS.OMS_ID_FIELD], self.oms_id)
            self.assertEqual(request_data[AP_CONSTANTS.ACCOUNT_ID_FIELD], self.account_id)
            self.assertEqual(request_data[AP_CONSTANTS.TIME_IN_FORCE_FIELD], AP_CONSTANTS.GTC_TIF)
            self.assertEqual(request_data[AP_CONSTANTS.CLIENT_ORDER_ID_FIELD], int(order.client_order_id))
            self.assertEqual(request_data[AP_CONSTANTS.QUANTITY_FIELD], Decimal("100"))
            self.assertEqual(request_data[AP_CONSTANTS.LIMIT_PRICE_FIELD], Decimal("10000"))
            self.assertEqual(request_data[AP_CONSTANTS.ORDER_TYPE_FIELD], AP_CONSTANTS.LIMIT_ORDER_TYPE)

        def validate_order_status_request(self, order: InFlightOrder, request_call: RequestCall):
            request_data = request_call.kwargs["params"]
            self.assertEqual(request_data[AP_CONSTANTS.OMS_ID_FIELD], self.oms_id)
            self.assertEqual(request_data[AP_CONSTANTS.ACCOUNT_ID_FIELD], self.account_id)
            self.assertEqual(request_data[AP_CONSTANTS.ORDER_ID_FIELD], int(order.exchange_order_id))

        def validate_order_cancelation_request(self, order: InFlightOrder, request_call: RequestCall):
            request_data = json.loads(request_call.kwargs["data"])
            self.assertEqual(request_data[AP_CONSTANTS.OMS_ID_FIELD], self.oms_id)
            self.assertEqual(request_data[AP_CONSTANTS.ACCOUNT_ID_FIELD], self.account_id)
            self.assertEqual(request_data[AP_CONSTANTS.CL_ORDER_ID_FIELD], int(order.client_order_id))
        ...
        def validate_trades_request(self, order: InFlightOrder, request_call: RequestCall):
            ...
            self.assertEqual(request_data[AP_CONSTANTS.OMS_ID_FIELD], self.oms_id)
            self.assertEqual(request_data[AP_CONSTANTS.ACCOUNT_ID_FIELD], self.account_id)
            self.assertEqual(request_data[AP_CONSTANTS.USER_ID_FIELD], self.user_id)
            self.assertEqual(request_data[AP_CONSTANTS.INSTRUMENT_ID_FIELD], instrument_id)
        ...
        def configure_successful_cancelation_response(
            ...
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_ORDER_CANCELATION_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        def configure_completely_filled_order_status_response(
            ...
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_ORDER_STATUS_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        def configure_partial_fill_trade_response(
            ...
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_TRADE_HISTORY_ENDPOINT)
            regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        def order_event_for_new_order_websocket_update(self, order: InFlightOrder) -> Dict:
            return {
                "i": 2,
                "m": 3,
                "n": AP_CONSTANTS.WS_ORDER_STATE_EVENT,
                "o": {
                    ...
                    "OrderState": AP_CONSTANTS.ACTIVE_ORDER_STATE,
                    "OrderType": "Limit",
                    "OrigClOrdId": 3,
        ...
        def order_event_for_canceled_order_websocket_update(self, order: InFlightOrder) -> Dict:
            return {
                ...
                "o": {
                    ...
                    "OrderFlag": "0",
                    "OrderId": int(order.exchange_order_id),
                    "OrderState": AP_CONSTANTS.CANCELED_ORDER_STATE,
                    "OrderType": "Limit",
                    "OrigClOrdId": 3,
                    "OrigOrderId": 18846132298,
                    "OrigQuantity": float(order.amount),
        ...
        def order_event_for_full_fill_websocket_update(self, order: InFlightOrder) -> Dict:
            return {
                "i": 4,
                "m": 3,
                "n": AP_CONSTANTS.WS_ORDER_TRADE_EVENT,
        ...
        def order_event_for_partially_filled_websocket_update(self, order: InFlightOrder):
            return {
                ...
                "m": 3,
                "n": AP_CONSTANTS.WS_ORDER_TRADE_EVENT,
                "o": self._order_fills_request_partial_fill_mock_response(order)[0],
            }
        ...
        def _initialize_auth(self, auth: AlphaPointAuth):
        ...
        def _order_status_request_completely_filled_mock_response(self, order: InFlightOrder) -> Any:
            return {
                ...
                "OrderState": AP_CONSTANTS.FULLY_EXECUTED_ORDER_STATE,
                "OrderType": "Limit",
                "OrigClOrdId": 1,
        ...
        def test_exchange_authenticates(self, mock_api):
            url = self.url_creator.get_rest_url(path_url=AP_CONSTANTS.REST_AUTH_ENDPOINT)
