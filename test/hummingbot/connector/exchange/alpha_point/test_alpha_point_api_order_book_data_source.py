...
from hummingbot.connector.exchange.alpha_point.alpha_point_web_utils import AlphaPointURLCreatorBase, build_api_factory
from hummingbot.connector.test_support.network_mocking_assistant import NetworkMockingAssistant
from hummingbot.core.data_type.order_book import OrderBook, OrderBookMessage
from hummingbot.core.data_type.order_book_message import OrderBookMessageType


class TestURCreator(AlphaPointURLCreatorBase):
...
class TestExchange(AlphaPointExchange):
    @property
    def name(self) -> str:
...
class AlphaPointAPIOrderBookDataSourceTest(unittest.TestCase):
    level = 0
    ...
    def setUp(self) -> None:
        ...
        connector = TestExchange(
            self.api_key, self.secret, self.user_id, trading_pairs=[self.trading_pair]
        )
        self.auth = AlphaPointAuth(api_key=self.api_key, secret_key=self.secret, user_id=self.user_id)
        self.initialize_auth()
        api_factory = build_api_factory(auth=self.auth)
        self.data_source = AlphaPointAPIOrderBookDataSource(
