...
def validate_price_source(value: str) -> Optional[str]:
    if value not in {"current_market", "external_market", "multi_market", "custom_api"}:
...
def on_validate_price_source(value: str):
    ...
    if value != "external_market" and value != "multi_market":
        pure_market_making_config_map["take_if_crossed"].value = None
    if value != "multi_market":
        pure_market_making_config_map["price_source_exchanges_and_markets"].value = None
    if value != "custom_api":
...
def validate_multi_market_price_sources(value: str):
    errors = []
    connector_and_market_pairs = []
    if "[" in value and "]" in value:
        exchange_and_market_list = ast.literal_eval(value)
    else:
        exchange_and_market_list = list(map(str.strip, value.split(",")))

    if len(exchange_and_market_list) < 2:
        errors.append("The multi_market price source requires at least two reference markets.")

    for exchange_and_market in exchange_and_market_list:
        if ":" in exchange_and_market:
            exchange, market = exchange_and_market.split(":")
            exchange_validation_result = validate_connector(exchange)
            if exchange_validation_result is None:
                market_validation_result = validate_market_trading_pair(market=exchange, value=market.upper())
                if market_validation_result is None:
                    connector_and_market_pairs.append((exchange, market))
                else:
                    errors.append(market_validation_result)
            else:
                errors.append(exchange_validation_result)
        else:
            errors.append(f"The value {exchange_and_market} is not a valid exchange:market pair")

    if len(errors) == 0:
        try:
            MultiMarketAssetPriceDelegate.validate_connectors_and_markets_configuration(connector_and_market_pairs)
        except ValueError as validation_error:
            errors.append(str(validation_error))

    return None if len(errors) == 0 else "\n".join(errors)
...
pure_market_making_config_map = {
    ...
    "price_source":
        ConfigVar(key="price_source",
                  prompt="Which price source to use? (current_market/external_market/multi_market/custom_api) >>> ",
                  type_str="str",
    ...
    "price_source_exchanges_and_markets":
        ConfigVar(key="price_source_exchanges_and_markets",
                  prompt="Enter a comma separated list of exchange:market to configure the price source >>> ",
                  required_if=lambda: pure_market_making_config_map.get("price_source").value == "multi_market",
                  type_str="list",
                  validator=validate_multi_market_price_sources),
    "order_override":
