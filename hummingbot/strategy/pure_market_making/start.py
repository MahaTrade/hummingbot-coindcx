...
def start(self):
    ...
    try:
        ...
        price_source_exchange_market_list = c_map.get("price_source_exchanges_and_markets").value
        ...
        elif price_source == "multi_market":
            exchange_market_tuples = []
            exchanges_and_markets = defaultdict(list)
            for exchange_and_market in price_source_exchange_market_list:
                exchange_name, trading_pair = exchange_and_market.split(":")
                exchange_market_tuples.append((exchange_name, trading_pair))
                exchanges_and_markets[exchange_name].append(trading_pair)
            connectors_registry = dict()
            for exchange_name, trading_pairs in exchanges_and_markets.items():
                connector = create_paper_trade_market(exchange_name=exchange_name, trading_pairs=trading_pairs)
                connectors_registry[exchange_name] = connector
            connectors_and_trading_pairs = [(connectors_registry[exchange_name], trading_pair)
                                            for exchange_name, trading_pair in exchange_market_tuples]
            asset_price_delegate = MultiMarketAssetPriceDelegate.for_markets(
                connectors_and_pairs=connectors_and_trading_pairs)
            for market in asset_price_delegate.all_markets:
                self.markets[f"{market.name}_paper_trade"] = market
