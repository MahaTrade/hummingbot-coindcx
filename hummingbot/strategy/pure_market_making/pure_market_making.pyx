...
cdef class PureMarketMakingStrategy(StrategyBase):
    ...
    def market_status_data_frame(self, market_trading_pair_tuples: List[MarketTradingPairTuple]) -> pd.DataFrame:
        ...
        if type(self._asset_price_delegate) in [OrderBookAssetPriceDelegate, MultiMarketAssetPriceDelegate]:
            market_books.extend(self.asset_price_delegate.all_markets_with_trading_pairs)
        for market, trading_pair in market_books:
            ...
            if market == self._market_info.market and self._inventory_cost_price_delegate is not None:
                ...
                try:
                    ref_price = market.get_price_by_type(trading_pair=trading_pair, price_type=self._price_type)
                except ValueError:
                    pass
    ...
    def format_status(self) -> str:
        ...
        strategy_price_source = "main market" if self._asset_price_delegate is None else "asset price delegate"
        lines.extend(["", f"  Current strategy price ({strategy_price_source}): {self.get_price():.4f}"])
