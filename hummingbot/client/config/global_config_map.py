...
def coingecko_extra_token_ids_on_validated(value: str):
    tokens = []
    if "[" in value and "]" in value:
        tokens = ast.literal_eval(value)
    else:
        tokens = list(map(str.lower, map(str.strip, value.split(","))))

    RateOracle.coingecko_extra_token_ids = tokens
...
main_config_map = {
    ...
    "rate_oracle_coingecko_extra_token_ids":
        ConfigVar(key="rate_oracle_coingecko_extra_token_ids",
                  prompt="List of comma-delimited CoinGecko token ids to always include in CoinGecko rates query "
                         "(e.g. frontier-token,pax-gold,rbtc)  >>> ",
                  type_str="list",
                  required_if=lambda: False,
                  on_validated=coingecko_extra_token_ids_on_validated,
                  default=[]),
    "global_token":
    ...
    "tables_format":
        ConfigVar(key="tables_format",
                  ...
                  default="psql"),
    "controller_server_enabled":
        ConfigVar(key="controller_server_enabled",
                  prompt=None,
                  type_str="bool",
                  required_if=lambda: False,
                  default=False),
