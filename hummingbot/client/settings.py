...
from hummingbot.core.utils.dynamic_imports import import_module_with_plugin_support
...
class ConnectorSetting(NamedTuple):
    ...
    def connector_module(self) -> ModuleType:

        if self.uses_gateway_generic_connector():
            module_name = self.module_name()
            module_package = "connector"
            parent_package = "hummingbot"
        else:
            module_name = self.module_name()
            module_package = self.base_name()
            parent_package = f"hummingbot.connector.{self.type.name.lower()}"

        module = import_module_with_plugin_support(
    ...
    def non_trading_connector_instance_with_default_configuration(
        ...
        connector_class = getattr(self.connector_module(), self.class_name())
...
class AllConnectorSettings:
    ...
    def create_connector_settings(cls):
        ...
        """
        all_connector_settings = {}
        connector_exceptions = ["paper_trade", "alpha_point"]
        ...
        for type_dir in type_dirs:
            ...
            for connector_dir in connector_dirs:
                if connector_dir.name.startswith("_") or connector_dir.name in connector_exceptions:
                    continue
                if connector_dir.name in all_connector_settings:
                    raise Exception(f"Multiple connectors with the same {connector_dir.name} name.")
                try:
                    util_module = import_module_with_plugin_support(
                        module_name=f"{connector_dir.name}_utils",
                        module_package=connector_dir.name,
                        parent_package=f"hummingbot.connector.{type_dir.name}"
                    )
                ...
                all_connector_settings[connector_dir.name] = ConnectorSetting(
                ...
                for domain in other_domains:
                    ...
                    parent = all_connector_settings[connector_dir.name]
                    all_connector_settings[domain] = ConnectorSetting(
                        name=domain,
                        type=parent.type,
        ...
        for connection_spec in gateway_connections_conf:
            ...
            all_connector_settings[market_name] = ConnectorSetting(
                name=market_name,
    ...
    def get_connector_settings(cls) -> Dict[str, ConnectorSetting]:
        tmp_settings = cls.create_connector_settings()
        if len(cls.all_connector_settings) == 0 or len(cls.all_connector_settings) < len(tmp_settings):
            cls.all_connector_settings = tmp_settings
