...
from . import coindcx_constants as CONSTANTS, coindcx_web_utils as web_utils
from .coindcx_api_order_book_data_source import CoindcxAPIOrderBookDataSource
from .coindcx_api_user_stream_data_source import CoindcxAPIUserStreamDataSource
...
class CoindcxExchange(ExchangePyBase):
    ...
    async def _place_order(
        ...
        try:
            ...
            failed_due_to_insufficient_balance = "insufficient balance" in error_message.lower()
            if failed_due_to_insufficient_balance:
                self.logger().exception(f"Unable to place order: {order_id}. Error Message: {error_message}")
            raise
            ...
            for client_order_id, data in order_updates:
                if "Order not found" in str(data):
                    ...
                    order_update = OrderUpdate(
                        ...
                        trading_pair=tracked_order.trading_pair,
                        update_timestamp=float(data["updated_at"]) * 1e-3,
                ...
                if channel == CONSTANTS.USER_BALANCE_EVENT_TYPE:
                    ...
                    for order_update in data:
                        ...
                        order_update = OrderUpdate(
                            ...
                            trading_pair=tracked_order.trading_pair,
                            update_timestamp=float(order_update["updated_at"]) * 1e-3,
