...
cdef class HuobiExchange(ExchangeBase):
    ...
    async def get_order_status(self, order: HuobiInFlightOrder) -> Dict[str, Any]:
        """
        Example:
        {
            "id": 59378,
    ...
    async def get_order_fills(self, order: HuobiInFlightOrder) -> Dict[str, Any]:
        """
        Example:
        [
            {
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                order_tasks.append(asyncio.create_task(self.get_order_status(order=tracked_order)))
                order_fills_tasks.append(asyncio.create_task(self.get_order_fills(order=tracked_order)))
            ...
            for order_update, order_fills, tracked_order in zip(order_updates, order_fills, tracked_orders):
                client_order_id = tracked_order.client_order_id

                if isinstance(order_fills, Exception):
                    self.logger().warning(f"Error fetching order fills for the order {client_order_id}: {order_fills}.")
                else:
                    for fill_data in order_fills:
                        trade_event = {
                            "orderId": fill_data["order-id"],
                            "tradePrice": fill_data["price"],
                            "tradeVolume": fill_data["filled-amount"],
                            "tradeId": fill_data["trade-id"],
                            "transactFee": fill_data["filled-fees"],
                            "feeCurrency": fill_data["fee-currency"],
                        }
                        updated = tracked_order.update_with_trade_update(trade_event)
                        if updated:
                            self.logger().info(
                                f"Filled {trade_event['tradeVolume']} out of {tracked_order.amount} of order "
                            ...
                            self.c_trigger_event(self.MARKET_ORDER_FILLED_EVENT_TAG,
                                                 OrderFilledEvent(
                                                     ...
                                                     Decimal(trade_event["tradePrice"]),
                                                     Decimal(trade_event["tradeVolume"]),
                                                     AddedToCostTradeFee(
                                                         flat_fees=[
                                                             TokenAmount(trade_event["feeCurrency"].upper(),
                ...
                if isinstance(order_update, HuobiAPIError):
                    err_code = order_update.error_payload.get("error").get("err-code")
                ...
                elif tracked_order.client_order_id in self._in_flight_orders:
                    # Only process order update if it has not been removed from tracking orders while requesting updates
    ...
    async def _process_order_update(self, order_update: Dict[str, Any]):
        ...
        previous_status = tracked_order.last_state
