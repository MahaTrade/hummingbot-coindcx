...
class AlphaPointAPIOrderBookDataSource(OrderBookTrackerDataSource):
    def __init__(
        self,
        trading_pairs: List[str],
        connector: 'AlphaPointExchange',
    ...
    async def _parse_trade_message(self, raw_message: List[Dict[int, Union[int, float]]], message_queue: asyncio.Queue):
        raise NotImplementedError  # AlphaPoint does not provide a public trades endpoint
    ...
    async def _parse_order_book_diff_message(
        ...
        update_id = self._nonce_provider.get_tracking_nonce(ts_us=ts_ms * 1e-3)
    ...
    def _get_bids_and_asks_from_snapshot(
        ...
        """AlphaPoint does not guarantee that the data is sorted in any way."""
        asks = []
        bids = []
        for row in snapshot:
