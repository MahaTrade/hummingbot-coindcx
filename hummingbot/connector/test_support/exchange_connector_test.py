    ...
    class ExchangeConnectorTests(ABC, TestCase):
        ...
        def test_update_order_status_when_order_partially_filled_and_cancelled(self, mock_api):
            ...
            if self.is_order_fill_http_update_included_in_status_update:
                ...
                self.assertEqual(self.expected_fill_trade_id, order_partially_filled_event.exchange_trade_id)
                self.assertEqual(self.expected_partial_fill_price, order_partially_filled_event.price)
                self.assertEqual(self.expected_partial_fill_amount, order_partially_filled_event.amount)
            ...
            order_url = self.configure_partial_cancelled_order_status_response(
                order=order,
                mock_api=mock_api)
            ...
            order_status_request = self._all_executed_requests(mock_api, order_url)[1]
            self.validate_auth_credentials_present(order_status_request)
            self.validate_order_status_request(
                order=order,
                request_call=order_status_request
        ...
        def test_user_stream_update_for_partially_cancelled_order(self, mock_api):
            ...
            order_event = self.order_event_for_new_order_websocket_update(order=order)
            partial_fill_order_event = self.order_event_for_partially_filled_websocket_update(order=order)
            partial_fill_trade_event = self.trade_event_for_partial_fill_websocket_update(order=order)
            cancel_order_event = self.order_event_for_partially_canceled_websocket_update(order=order)

            mock_queue = AsyncMock()
            event_messages = []
            if order_event:
                event_messages.append(order_event)
            if partial_fill_order_event:
                event_messages.append(partial_fill_order_event)
            if partial_fill_trade_event:
                event_messages.append(partial_fill_trade_event)
            if cancel_order_event:
                event_messages.append(cancel_order_event)
            event_messages.append(asyncio.CancelledError)
            ...
            self.assertTrue(
                self.is_logged(
                    ...
                    f"{str(self.expected_partial_fill_amount)}/{str(order.amount)} {self.base_asset} has been filled."
                )
            )
