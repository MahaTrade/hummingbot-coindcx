...
class RateOracle(NetworkBase):
    ...
    coingecko_extra_token_ids: List[str] = []
    ...
    coingecko_extra_tokens_url = "https://api.coingecko.com/api/v3/coins/markets?ids={}&vs_currency={}"
    ...
    async def get_coingecko_prices(cls, vs_currency: str) -> Dict[str, Decimal]:
        ...
        tasks.append(asyncio.get_event_loop().create_task(cls.get_coingecko_extra_token_prices(vs_currency)))
        task_results = await safe_gather(*tasks, return_exceptions=True)
        for task_result in task_results:
            if isinstance(task_result, Exception):
    ...
    @classmethod
    async def get_coingecko_extra_token_prices(cls, vs_currency: str) -> Dict[str, Decimal]:
        """
        ...
        """
        results = {}
        if len(cls.coingecko_extra_token_ids) > 0:
            client = await cls._http_client()
            tokens_parameter_value = ",".join(map(str.lower, cls.coingecko_extra_token_ids))
            url = cls.coingecko_extra_tokens_url.format(tokens_parameter_value, vs_currency)
            async with client.request("GET", url) as resp:
                records = await resp.json(content_type=None)
                for record in records:
                    pair = f'{record["symbol"].upper()}-{vs_currency.upper()}'
